    ...
    private static class PolymerReactor {
        ...
        int calculate() {
            ...
            for (String letter : "abcdefghijklmnopqrstuvwxyz".split("")) {
                String adjustedInput = input.replaceAll(letter, "").replaceAll(letter.toUpperCase(), "");
                int letterResult = createFullyReacted(adjustedInput);

                if (letterResult < smallestResult)
                    smallestResult = letterResult;
            }

            return smallestResult;
        }

        private int createFullyReacted(String value) {
